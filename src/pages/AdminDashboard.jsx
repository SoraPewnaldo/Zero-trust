...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [logsResponse, statsResponse, usersResponse] = await Promise.all([api.admin.getScanLogs({
        limit: 100,
      ...
      const realLogs = logsResponse.scans.map(scan => ({
        id: scan._id,
      ...
      setStats({
        ...
        lastDecision: null,
        // Not provided by stats endpoint
    ...
    try {
      ...
      const userDetail = {
        ...
        deviceInfo: data.devices[0] ? {
          ...
          location: 'Unknown',
          // Backend doesn't have location yet
        ...
        logs: data.scans.map(scan => ({
          id: scan._id || 'unknown',
          userId: typeof scan.userId === 'object' && scan.userId && '_id' in scan.userId ? scan.userId._id : typeof scan.userId === 'string' ? scan.userId : 'unknown',
          username: typeof scan.userId === 'object' && scan.userId && 'username' in scan.userId ? scan.userId.username : scan.username || 'Unknown',
          role: typeof scan.userId === 'object' && scan.userId && 'role' in scan.userId ? scan.userId.role : scan.role || 'unknown',
          deviceId: typeof scan.deviceId === 'object' && scan.deviceId && 'deviceName' in scan.deviceId ? scan.deviceId.deviceName : typeof scan.deviceId === 'string' ? scan.deviceId : 'Unknown',
          trustScore: scan.trustScore,
          decision: scan.decision,
          timestamp: scan.createdAt || scan.timestamp,
          deviceInfo: {
  ...
  const sortedLogs = [...logs].sort((a, b) => {
    ...
    if (sortField === 'timestamp') {
      ...
      valA = (a.resource ?? '').toLowerCase();
      valB = (b.resource ?? '').toLowerCase();
    } else if (sortField === 'trustScore') {
      valA = a.trustScore;
      valB = b.trustScore;
    } else if (sortField === 'decision') {
      valA = a.decision.toLowerCase();
      valB = b.decision.toLowerCase();
    }
  ...
  const SortHeader = ({
    field,
    label
  }) => <th className="text-[10px] font-mono text-white/50 tracking-wider py-2 pr-4 cursor-pointer select-none hover:text-white/80 transition-colors" onClick={() => handleSort(field)}>
  ...
  const scoreDistribution = (() => {
    ...
    logs.forEach(l => {
      if (l.trustScore <= 20) buckets[0].count++;else if (l.trustScore <= 40) buckets[1].count++;else if (l.trustScore <= 60) buckets[2].count++;else if (l.trustScore <= 80) buckets[3].count++;else buckets[4].count++;
    });
    return buckets;
  })();
  const COLORS = ['rgba(255,255,255,0.8)', 'rgba(255,255,255,0.5)', 'rgba(255,255,255,0.25)'];
  return <DashboardLayout title="ADMIN DASHBOARD" subtitle="ORGANIZATION ACCESS MANAGEMENT">
      {/* Summary Stat Cards */}
      ...
      {/* Threat Alerts */}
      {threats.length > 0 && <div className="border border-white/20 p-4 lg:p-6 mb-6">
          ...
          <div className="space-y-2">
            {threats.slice(0, 4).map(t => <div key={t.id} className="flex items-center gap-4 p-3 border border-white/10">
      ...
      <div className="flex items-center gap-0 mb-6 border-b border-white/20">
        {['logs', 'users', 'analytics'].map(tab => <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 text-[10px] font-mono tracking-wider transition-all duration-200 border-b-2 ${activeTab === tab ? 'text-white border-white' : 'text-white/40 border-transparent hover:text-white/60'}`}>
            {tab.toUpperCase()}
          </button>)}
      </div>
      ...
      {activeTab === 'logs' && <>
          ...
          <div className="border border-white/20 p-4 lg:p-6 mb-6">
            ...
            <div className="flex flex-col lg:flex-row gap-3">
              <div className="flex-1">
                ...
                <input type="text" value={filterUser} onChange={e => setFilterUser(e.target.value)} className="w-full bg-transparent border border-white/20 text-white font-mono text-xs px-3 py-2 focus:outline-none focus:border-white/50 transition-colors placeholder:text-white/15" placeholder="filter by user" />
              </div>
              <div>
              ...
              <div>
                ...
                <select value={filterDecision} onChange={e => setFilterDecision(e.target.value)} className="w-full bg-black border border-white/20 text-white font-mono text-xs px-3 py-2 focus:outline-none focus:border-white/50">
                  <option value="">ALL</option>
              ...
              <div>
                ...
                <select value={filterResource} onChange={e => setFilterResource(e.target.value)} className="w-full bg-black border border-white/20 text-white font-mono text-xs px-3 py-2 focus:outline-none focus:border-white/50">
                  <option value="">ALL</option>
          ...
          <div className="border border-white/20 p-4 lg:p-6">
            ...
            {loading ? <div className="text-xs font-mono text-white/40 animate-pulse">LOADING RECORDS...</div> : logs.length === 0 ? <div className="text-xs font-mono text-white/30">NO RECORDS MATCH FILTERS</div> : <>
                {/* Desktop Table */}
                    ...
                    <tbody>
                      {sortedLogs.map(entry => <>
                          <tr key={entry.id} className="border-b border-white/5">
                            ...
                            <td className="py-2">
                              {entry.factors && entry.factors.length > 0 && <button onClick={() => setExpandedLog(expandedLog === entry.id ? null : entry.id)} className="text-[10px] font-mono text-white/50 hover:text-white/80 border border-white/15 px-2 py-0.5 transition-colors">
                                  {expandedLog === entry.id ? 'HIDE' : 'FACTORS'}
                                </button>}
                            </td>
                          </tr>
                          {expandedLog === entry.id && entry.factors && <tr key={`${entry.id}-factors`}>
                              <td colSpan={8} className="py-2 px-4">
                                <div className="border border-white/10 p-3 space-y-1.5">
                                  {entry.factors.map((f, idx) => <div key={idx} className="flex items-center gap-3 text-[10px] font-mono">
                ...
                <div className="md:hidden space-y-3">
                  {logs.map(entry => <div key={entry.id} className="border border-white/10 p-3 space-y-2">
                      ...
                      {entry.factors && entry.factors.length > 0 && <button onClick={() => setExpandedLog(expandedLog === entry.id ? null : entry.id)} className="text-[10px] font-mono text-white/50 hover:text-white/80 border border-white/15 px-2 py-0.5 transition-colors w-full text-center">
                          {expandedLog === entry.id ? 'HIDE FACTORS' : 'SHOW FACTORS'}
                        </button>}
                      {expandedLog === entry.id && entry.factors && <div className="border-t border-white/10 pt-2 space-y-1">
                          {entry.factors.map((f, idx) => <div key={idx} className="flex items-center justify-between text-[10px] font-mono">
      ...
      {/* Users Tab - List */}
      {activeTab === 'users' && !selectedUser && <div className="border border-white/20 p-4 lg:p-6">
          <div className="flex items-center gap-2 mb-4">
            <div className="w-6 h-px bg-white/40"></div>
            ...
            <button onClick={() => setShowAddEmployee(true)} className="ml-4 px-3 py-1.5 bg-white text-black text-[10px] font-mono font-bold flex items-center gap-2 hover:bg-white/90 transition-colors">
          ...
          <div className="overflow-x-auto hidden md:block">
            <table className="w-full text-left">
              ...
              <tbody>
                {users.map(u => <tr key={u.userId} className="border-b border-white/5">
                    ...
                    <td className="py-2 pr-4">
                      {u.lastDecision ? <span className={`px-2 py-0.5 text-[9px] font-mono border border-white/15 ${decisionStyle(u.lastDecision)}`}>
                          {u.lastDecision.toUpperCase()}
                        </span> : <span className="text-xs font-mono text-white/30">—</span>}
                    </td>
          ...
          {/* Mobile Card Layout */}
          <div className="md:hidden space-y-3">
            {users.map(u => <div key={u.userId} className="border border-white/10 p-3 space-y-2">
                <div className="flex items-center justify-between">
      ...
      {/* User Detail View */}
      {activeTab === 'users' && selectedUser && <div>
          <button onClick={() => setSelectedUser(null)} className="flex items-center gap-2 text-[10px] font-mono text-white/50 hover:text-white/80 mb-4 transition-colors">
          ...
          <div className="border border-white/20 p-4 lg:p-6 mb-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              {[{
            label: 'TOTAL SCANS',
            value: selectedUser.stats.totalScans
          }, {
            label: 'AVG SCORE',
            value: selectedUser.stats.avgScore
          }, {
            label: 'ALLOWED',
            value: selectedUser.stats.allowCount
          }, {
            label: 'MFA REQ',
            value: selectedUser.stats.mfaCount
          }, {
            label: 'BLOCKED',
            value: selectedUser.stats.blockedCount
          }].map(s => <div key={s.label} className="border border-white/10 p-3">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            <div className="border border-white/20 p-4 lg:p-6">
              ...
              label: 'OS',
              value: selectedUser.deviceInfo.os
            }, {
              icon: <Monitor size={14} className="text-white/40" />,
              label: 'BROWSER',
              value: selectedUser.deviceInfo.browser
            }, {
              icon: <Globe size={14} className="text-white/40" />,
              label: 'IP ADDRESS',
              value: selectedUser.deviceInfo.ip
            }, {
              icon: <Globe size={14} className="text-white/40" />,
              label: 'LOCATION',
              value: selectedUser.deviceInfo.location
            }, {
              icon: <Clock size={14} className="text-white/40" />,
              label: 'LAST SEEN',
              value: new Date(selectedUser.deviceInfo.lastSeen).toLocaleString()
            ...
            <div className="lg:col-span-2 border border-white/20 p-4 lg:p-6">
              ...
              <div className="space-y-2">
                {selectedUser.recommendations.map(rec => <div key={rec.id} className="flex items-start gap-3 p-3 border border-white/10">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className={`px-2 py-0.5 text-[9px] font-mono border border-white/15 ${rec.priority === 'high' ? 'text-white bg-white/15' : rec.priority === 'medium' ? 'text-white/70 bg-white/[0.08]' : 'text-white/50 bg-white/5'}`}>{rec.priority.toUpperCase()}</span>
                      </div>
          ...
          <div className="border border-white/20 p-4 lg:p-6">
            ...
            {/* Mobile Card Layout */}
            <div className="md:hidden space-y-3">
              {selectedUser.logs.map(entry => <div key={entry.id} className="border border-white/10 p-3 space-y-2">
                  <div className="flex items-center justify-between">
      ...
      {/* Analytics Tab */}
      {activeTab === 'analytics' && <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="border border-white/20 p-4 lg:p-6">
            ...
            <div className="h-[250px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={decisionChartData} cx="50%" cy="50%" outerRadius={80} dataKey="value" stroke="rgba(255,255,255,0.2)" strokeWidth={1} label={({
                name,
                value
              }) => `${name}: ${value}`}>
                    {decisionChartData.map((_, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                  </Pie>
      ...
      {showAddEmployee && <EmployeeForm onClose={() => setShowAddEmployee(false)} onSuccess={() => {
      fetchData();
    }} api={api} />}

      <ConfirmationModal isOpen={confirmDelete.show} title="CONFIRM TERMINATION" message="Are you sure you want to fire/delete this employee? This action cannot be undone." confirmText="FIRE EMPLOYEE" cancelText="CANCEL" variant="danger" onConfirm={confirmDeleteUser} onCancel={() => setConfirmDelete({
      show: false,
      userId: null
    })} />
    </DashboardLayout>;
