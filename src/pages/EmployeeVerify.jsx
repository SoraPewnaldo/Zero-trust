...
export default function EmployeeVerify() {
  ...
  return <DashboardLayout title="ACCESS VERIFICATION" subtitle="IDENTITY & DEVICE TRUST CHECK REQUIRED">
      <div className="max-w-2xl mx-auto">
        <div className="border border-white/20 p-6 mb-6 text-center">
          ...
          <h2 className="text-lg font-mono text-white tracking-wider mb-2">
            {!scanResult ? 'VERIFICATION REQUIRED' : isAllowed ? 'ACCESS GRANTED' : scanResult.decision === 'mfa_required' ? 'ADDITIONAL VERIFICATION NEEDED' : 'ACCESS DENIED'}
          </h2>
          <p className="text-[10px] font-mono text-white/40">
            {!scanResult ? 'Complete a device trust scan to access your dashboard' : isAllowed ? 'Your identity and device have been verified. You may proceed.' : scanResult.decision === 'mfa_required' ? 'Your trust score requires step-up authentication.' : 'Your device does not meet security requirements.'}
          </p>
        </div>

        {isAllowed && <div className="text-center mb-6">
            <button onClick={() => {
          ...
          if (targetId === 'admin-dashboard') {
            ...
            const isAdminLike = ['super_admin', 'security_analyst', 'resource_manager'].includes(user?.role);
            navigate(isAdminLike ? '/admin' : '/employee');
          }
        ...
        {!isAllowed && <div className="border border-white/20 p-4 lg:p-6 mb-6">
            ...
            <div className="mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {resources.map(r => <button key={r._id} onClick={() => setSelectedResourceId(r._id)} className={`px-3 py-1.5 text-[10px] font-mono border transition-all duration-200 ${selectedResourceId === r._id ? 'border-white text-white bg-white/20' : 'border-white/10 text-white/40 hover:text-white/60 hover:border-white/40'}`}>
                    {r.name.toUpperCase()}
                  </button>)}
              </div>
            </div>

            <button onClick={handleScan} disabled={scanning} className="w-full relative px-6 py-2.5 bg-transparent text-white font-mono text-sm border border-white hover:bg-white hover:text-black transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed group">
              <span className="absolute -top-1 -left-1 w-2 h-2 border-t border-l border-white opacity-0 group-hover:opacity-100 transition-opacity"></span>
              <span className="absolute -bottom-1 -right-1 w-2 h-2 border-b border-r border-white opacity-0 group-hover:opacity-100 transition-opacity"></span>
        ...
        {scanResult && <div className="border border-white/20 p-4 lg:p-6 mb-6">
            <div className="flex items-center gap-2 mb-4">
              <div className="w-6 h-px bg-white/40"></div>
            ...
            <div className="mb-4">
              ...
              <div className="w-full h-2 bg-white/10 border border-white/20">
                <div className={`h-full transition-all duration-1000 ease-out ${scanResult.trustScore >= 60 ? 'bg-green-500' : 'bg-red-500'}`} style={{
              width: `${scanResult.trustScore}%`
            }} />
              </div>
            </div>
            ...
            {scanResult.decision === 'mfa_required' && !scanResult.mfaVerified && <div className="border border-yellow-500/20 p-4 space-y-3 mb-4 bg-yellow-500/5">
                <div className="flex items-center gap-2">
                ...
                <button onClick={handleMfaVerify} disabled={verifying} className="w-full py-2 bg-yellow-500/10 text-yellow-400 border border-yellow-500/30 hover:bg-yellow-500/20 transition-all font-mono text-[10px]">
                  {verifying ? '◐ VERIFYING...' : 'VERIFY WITH MFA'}
                </button>
              </div>}

            {scanResult.factors && scanResult.factors.length > 0 && <div className="space-y-1.5 mt-4">
                <div className="text-[9px] font-mono text-white/30 tracking-widest mb-2">DECISION FACTORS</div>
                {scanResult.factors.map((factor, idx) => <div key={idx} className="flex items-center gap-3 p-2 border border-white/5 bg-white/[0.02]">
