  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <div className={`border ${variantStyles[variant]} max-w-md w-full p-6 shadow-2xl`}>
                ...
                <div className="flex items-center gap-3 justify-end">
                    <button onClick={onCancel} className="px-4 py-2 text-xs font-mono text-white/70 hover:text-white border border-white/20 hover:border-white/40 transition-colors tracking-wider">
                        {cancelText}
                    </button>
                    <button onClick={onConfirm} className={`px-4 py-2 text-xs font-mono font-bold transition-colors tracking-wider ${buttonStyles[variant]}`}>
                        {confirmText}
                    </button>
                </div>
            </div>
