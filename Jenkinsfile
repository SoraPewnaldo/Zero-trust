                ...
                script {
                    ...
                    withCredentials([sshUserPrivateKey(credentialsId: 'ec2-ssh-key', keyFileVariable: 'PEM_KEY')]) {
                        ...
                        powershell """
                            ...
                            Set-Acl \$path \$acl
                        """
                        bat "ssh -i %PEM_KEY% -o StrictHostKeyChecking=no ubuntu@15.207.15.101 \"cd soraiam && git fetch origin && git reset --hard origin/main && docker-compose -f docker-compose.prod.yml up -d --build --force-recreate --no-deps frontend backend trust-engine && docker-compose -f docker-compose.prod.yml up -d --no-recreate mongodb\""
                    }
                }
            }
        }
