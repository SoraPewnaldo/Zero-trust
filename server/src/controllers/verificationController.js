...
export const initiateScan = async (req, res) => {
  try {
    ...
    if (!resource.allowedRoles.includes(req.user.role)) {
      res.status(403).json({
        error: 'User role not authorized for this resource'
      });
      return;
    }
    ...
    try {
      ...
      if (engineDetails.os_updated) {
        ...
        factors.push({
          ...
          weight: 20,
          impact: 0,
          details: 'OS update status unknown or pending'
        });
      }
      if (!engineDetails.risky_ports_found) {
        ...
        factors.push({
          ...
          weight: 20,
          impact: 0,
          details: 'Risky open ports detected'
        });
      }
      factors.push({
        name: 'Scan Freshness',
...
export const verifyMFA = async (req, res) => {
  try {
    ...
    if (!scanId || !mfaCode) {
      res.status(400).json({
        error: 'Scan ID and MFA code required'
      });
      return;
    }
    ...
    if (!scanResult.mfaRequired) {
      res.status(400).json({
        error: 'MFA not required for this scan'
      });
      return;
    }
...
export const getScanStatus = async (req, res) => {
  try {
    ...
    const scanResult = await ScanResult.findOne({
      scanId
    }).populate('resourceId', 'name sensitivity').populate('deviceId', 'deviceName deviceType');
    if (!scanResult) {
      res.status(404).json({
        error: 'Scan not found'
      });
